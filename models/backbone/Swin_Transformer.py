...
class WindowAttention(nn.Module):
	...
	def forward(self, x, mask=None):
		...
		if mask is not None:
			...
			attnmap = attn.detach()  # (B_, heads, N, N)
...
class SwinTransformerBlock(nn.Module):
	...
	def forward(self, x):
		...
		attn_windows, attnmap = self.attn(x_windows, mask=self.attn_mask)  # nW*B, window_size*window_size, C
		...
		for i, blk in enumerate(self.blocks):
			if self.use_checkpoint:
				...
				if i == len(self.blocks) - 1:
					attnmaps.append(attnmap.detach())
		return x, attnmaps
...
class SwinTransformer(nn.Module):
	...
	def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
	             ...
	             use_checkpoint=False, fused_window_process=False, cross_layer=True,
	             **kwargs):
		super().__init__()
	...
	def forward_features(self, x):
		...
		for idx,layer in enumerate(self.layers):
			...
			if self.cross_layer:
				layer_x.append(x)
				attnmaplist.append(attnmap)

		if self.cross_layer:
			return layer_x,attnmaplist
		else:
			x = self.norm(x)  # B L C
			return x,attnmaplist



	def forward(self, x):
		x,attnmaplist = self.forward_features(x)
