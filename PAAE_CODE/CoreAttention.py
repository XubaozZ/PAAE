...
class ChannelEnhancement(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.path1_pwc(x1)
        x1 = self.path1_dwc(x1)

        x2_half = self.path2_pwc(x2[:, :self.half_channels])
        x2_res = x2[:, self.half_channels:]
        x2 = torch.cat([x2_half, x2_res], dim=1)
...
class BiCrossAttention(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        gated_q1 = self.apply_global_gating(q1, self.gate_q)
        ...
        def attn(q, k, v):
            B, H, C, Hh, Ww = q.shape
            q = q.view(B, H, C, Hh * Ww).permute(0, 1, 3, 2)
            k = k.view(B, H, C, Hh * Ww)
            v = v.view(B, H, C, Hh * Ww).permute(0, 1, 3, 2)
...
class OutputWrapper(nn.Module):
    ...
    def forward(self, x):
        return self.enhancer.out_features
