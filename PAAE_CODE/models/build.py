...
from models.pae import MultiPartsSampling
...
def load_pretrained(config, model):
	...
	if 'head.weight' in state_dict:
		print(f"Deleting pretrained head.weight: {state_dict['head.weight'].shape} -> {model.head.weight.shape}")
		del state_dict['head.weight']
	if 'head.bias' in state_dict:
		print(f"Deleting pretrained head.bias: {state_dict['head.bias'].shape} -> {model.head.bias.shape}")
		del state_dict['head.bias']
